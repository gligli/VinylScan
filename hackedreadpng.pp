 ...
 **********************************************************************}
{$mode objfpc}{$h+}{$RANGECHECKS OFF}{$OVERFLOWCHECKS OFF}
unit hackedreadpng;
...
Type
  ...
  THackedReaderPNG = class (TFPCustomImageReader)
    private
...
constructor THackedReaderPNG.create;
begin
  inherited;
  chunk.acapacity := 0;
  chunk.data := nil;
...
destructor THackedReaderPNG.destroy;
begin
  with chunk do
    if acapacity > 0 then
      freemem (data);
...
procedure THackedReaderPNG.ReadChunk;
...
function THackedReaderPNG.GetHeaderByte(AIndex: Integer): Byte;
begin
  With FHeader do
  Case aIndex of
     0 : Result:=BitDepth;
...
function THackedReaderPNG.GetIndexed: Boolean;
begin
  Result:=ColorType=3;
end;

function THackedReaderPNG.GetUseAlpha: Boolean;
begin
  Result:=ColorType in [4,6]; // Can also be in 3, but that would require scanning the palette
end;

function THackedReaderPNG.GetWordSized: Boolean;
begin
  Result:=BitDepth=16;
end;

function THackedReaderPNG.GetGrayScale: Boolean;
begin
  Result:=ColorType in [0,4];
end;

procedure THackedReaderPNG.HandleData;
...
procedure THackedReaderPNG.HandleAlpha;
...
procedure THackedReaderPNG.HandlePalette;
var r : longword;
    c : TFPColor;
    t : word;
begin
...
procedure THackedReaderPNG.SetPalettePixel (x,y:integer; CD : TColordata);
begin  // both PNG and palette have palette
  TheImage.Pixels[x,y] := CD;
end;

procedure THackedReaderPNG.SetPalColPixel (x,y:integer; CD : TColordata);
begin  // PNG with palette, Img without
  TheImage.Colors[x,y] := ThePalette[CD];
end;

procedure THackedReaderPNG.SetColorPixel (x,y:integer; CD : TColordata);
var c : TFPColor;
...
procedure THackedReaderPNG.SetColorTrPixel (x,y:integer; CD : TColordata);
var c : TFPColor;
...
function THackedReaderPNG.CurrentLine(x:longword):byte;
begin
  result := FCurrentLine^[x];
end;

function THackedReaderPNG.PrevSample (x:longword): byte;
begin
  if x < byteWidth then
    result := 0
  else
...
function THackedReaderPNG.PreviousLine (x:longword) : byte;
begin
  result := FPreviousline^[x];
end;

function THackedReaderPNG.PrevLinePrevSample (x:longword): byte;
begin
  if x < byteWidth then
    result := 0
  else
...
function THackedReaderPNG.DoFilter(LineFilter:byte;index:longword; b:byte) : byte;
var diff : byte;
  procedure FilterSub;
  begin
...
function THackedReaderPNG.DecideSetPixel : TSetPixelProc;
begin
  if Pltte then
    if TheImage.UsePalette then
...
function THackedReaderPNG.CalcX (relX:integer) : integer;
begin
  result := StartX + (relX * deltaX);
end;

function THackedReaderPNG.CalcY (relY:integer) : integer;
begin
  result := StartY + (relY * deltaY);
end;

function THackedReaderPNG.CalcColor: TColorData;
var cd : longword;
    r : word;
    b : pbyte;
begin
...
procedure THackedReaderPNG.HandleScanLine (const y : integer; const ScanLine : PByteArray);
var x, rx : integer;
    c : TColorData;
begin
  UsingBitGroup := 0;
...
function THackedReaderPNG.ColorGray1 (CD:TColorDAta) : TFPColor;
begin
  if CD = 0 then
    result := colBlack
  else
...
function THackedReaderPNG.ColorGray2 (CD:TColorDAta) : TFPColor;
var c : word;
begin
  c := CD and 3;
  c := c + (c shl 2);
...
function THackedReaderPNG.ColorGray4 (CD:TColorDAta) : TFPColor;
var c : word;
begin
  c := CD and $F;
  c := c + (c shl 4);
...
function THackedReaderPNG.ColorGray8 (CD:TColorDAta) : TFPColor;
var c : word;
begin
  c := CD and $FF;
  c := c + (c shl 8);
...
function THackedReaderPNG.ColorGray16 (CD:TColorDAta) : TFPColor;
var c : word;
begin
  c := CD and $FFFF;
  with result do
...
function THackedReaderPNG.ColorGrayAlpha8 (CD:TColorData) : TFPColor;
var c : word;
begin
  c := CD and $00FF;
  c := c + (c shl 8);
...
function THackedReaderPNG.ColorGrayAlpha16 (CD:TColorData) : TFPColor;
var c : word;
begin
...
function THackedReaderPNG.ColorColor8 (CD:TColorData) : TFPColor;
var c : word;
begin
  with result do
    begin
...
function THackedReaderPNG.ColorColor16 (CD:TColorData) : TFPColor;
begin
  with result do
    begin
    red := CD and $FFFF;
...
function THackedReaderPNG.ColorColorAlpha8 (CD:TColorData) : TFPColor;
var c : word;
begin
  with result do
    begin
...
function THackedReaderPNG.ColorColorAlpha16 (CD:TColorData) : TFPColor;
begin
  with result do
    begin
    red := CD and $FFFF;
...
procedure THackedReaderPNG.DoDecompress;
...
procedure THackedReaderPNG.HandleChunk;
begin
  case chunk.AType of
...
procedure THackedReaderPNG.HandleUnknown;
begin
...
procedure THackedReaderPNG.InternalRead (Str:TStream; Img:TFPCustomImage);
begin
...
class function THackedReaderPNG.InternalSize(Str: TStream): TPoint;
var
...
function  THackedReaderPNG.InternalCheck (Str:TStream) : boolean;
