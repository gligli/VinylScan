...
type
  ...
  THackedWriterPNG = class (TFPCustomImageWriter)
    private
...
constructor THackedWriterPNG.create;
begin
  inherited;
  Fchunk.acapacity := 0;
  Fchunk.data := nil;
...
destructor THackedWriterPNG.destroy;
begin
...
procedure THackedWriterPNG.WriteChunk;
...
procedure THackedWriterPNG.SetChunkLength(aValue : longword);
begin
  with Fchunk do
    begin
    alength := aValue;
...
procedure THackedWriterPNG.SetChunkType (ct : TChunkTypes);
begin
  with Fchunk do
    begin
    aType := ct;
...
procedure THackedWriterPNG.SetChunkType (ct : TChunkCode);
begin
  with FChunk do
    begin
    ReadType := ct;
...
function THackedWriterPNG.CurrentLine(x:longword):byte;
begin
  result := FCurrentLine^[x];
end;

function THackedWriterPNG.PrevSample (x:longword): byte;
begin
  if x < byteWidth then
    result := 0
  else
...
function THackedWriterPNG.PreviousLine (x:longword) : byte;
begin
  result := FPreviousline^[x];
end;

function THackedWriterPNG.PrevLinePrevSample (x:longword): byte;
begin
  if x < byteWidth then
    result := 0
  else
...
function THackedWriterPNG.DoFilter(LineFilter:byte;index:longword; b:byte) : byte;
var diff : byte;
  procedure FilterSub;
  begin
...
procedure THackedWriterPNG.DetermineHeader (var AHeader : THeaderChunk);
var c : integer;
...
procedure THackedWriterPNG.WriteIHDR;
begin
  // signature for PNG
...
function THackedWriterPNG.ColorDataGrayB(color:TFPColor) : TColorData;
var t : word;
begin
...
function THackedWriterPNG.ColorDataGrayW(color:TFPColor) : TColorData;
begin
  result := CalculateGray (color);
end;

function THackedWriterPNG.ColorDataGrayAB(color:TFPColor) : TColorData;
begin
  result := ColorDataGrayB (color);
  result := (result shl 8) and hi(color.Alpha);
end;

function THackedWriterPNG.ColorDataGrayAW(color:TFPColor) : TColorData;
begin
  result := ColorDataGrayW (color);
  result := (result shl 16) and color.Alpha;
end;

function THackedWriterPNG.ColorDataColorB(color:TFPColor) : TColorData;
begin
  with color do
    result := hi(red) + (green and $FF00) + (hi(blue) shl 16);
end;

function THackedWriterPNG.ColorDataColorW(color:TFPColor) : TColorData;
begin
  with color do
    result := red + (green shl 16) + (qword(blue) shl 32);
end;

function THackedWriterPNG.ColorDataColorAB(color:TFPColor) : TColorData;
begin
  with color do
    result := hi(red) + (green and $FF00) + (hi(blue) shl 16) + (hi(alpha) shl 24);
end;

function THackedWriterPNG.ColorDataColorAW(color:TFPColor) : TColorData;
begin
  with color do
...
function THackedWriterPNG.GetColorPixel (x,y:longword) : TColorData;
begin
  result := FFmtColor (TheImage[x,y]);
  //result := ConvertColorToData(TheImage.Colors[x,y],CFmt);
end;

function THackedWriterPNG.GetPalettePixel (x,y:longword) : TColorData;
begin
  result := TheImage.Pixels[x,y];
end;

function THackedWriterPNG.GetColPalPixel (x,y:longword) : TColorData;
begin
  result := ThePalette.IndexOf (TheImage.Colors[x,y]);
end;

function THackedWriterPNG.DecideGetPixel : TGetPixelFunc;
begin
...
procedure THackedWriterPNG.WritePLTE;
var r,t : integer;
    c : TFPColor;
begin
  with ThePalette do
...
procedure THackedWriterPNG.InitWriteIDAT;
begin
...
procedure THackedWriterPNG.FinalWriteIDAT;
begin
  ZData.Free;
...
function THackedWriterPNG.DetermineFilter (Current, Previous:PByteArray; linelength:longword) : byte;
begin
  result := 0;
end;

procedure THackedWriterPNG.FillScanLine (y : integer; ScanLine : pByteArray);
var r, x : integer;
    cd : TColorData;
    index : longword;
    b : byte;
...
procedure THackedWriterPNG.GatherData;
var x,y : integer;
    lf : byte;
begin
...
procedure THackedWriterPNG.WriteCompressedData;
var l : longword;
begin
...
procedure THackedWriterPNG.WriteIDAT;
begin
  InitWriteIDAT;
  GatherData;
  WriteCompressedData;
...
procedure THackedWriterPNG.WritetRNS;
...
procedure THackedWriterPNG.WriteTexts;
begin
end;

procedure THackedWriterPNG.WriteIEND;
begin
  SetChunkLength(0);
  SetChunkType (ctIEND);
  WriteChunk;
...
procedure THackedWriterPNG.InternalWrite (Str:TStream; Img:TFPCustomImage);
begin
  WriteIHDR;
